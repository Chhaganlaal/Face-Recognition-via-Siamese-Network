...
def inception_model(X_input, name):
    ...
    # Inception 1: a/b/c
    X = inception_block_1a(X, name)
    X = inception_block_1b(X, name)
    X = inception_block_1c(X, name)

    # Inception 2: a/b
    X = inception_block_2a(X, name)
    X = inception_block_2b(X, name)

    # Inception 3: a/b
    X = inception_block_3a(X, name)
    X = inception_block_3b(X, name)
...
def get_training_model(input_shape):
    ...
    model = inception_model(Input(input_shape), 'name')
...
def recognize(model, encodings, image):
